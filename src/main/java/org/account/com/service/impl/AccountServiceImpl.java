...
public class AccountServiceImpl implements AccountService {
    ...
    public ResponseResult getById(String id) {
        logger.info(Sl4jToString.info(1,
                ...
                200,
                null));
        BusinessAccountModel model = mapper.getById(id);
        if (model != null) {
            result.setSuccess(true);
    ...
    public ResponseResult getByAccount(String account) {
        logger.info(Sl4jToString.info(1,
                ...
                200,
                null));
        BusinessAccountModel model = mapper.getByAccount(account);
        if (model != null) {
            result.setSuccess(true);
            result.setData(model);
            result.setCode(200);
    ...
    public ResponseResult findAllPage(int pageNow, int pageSize, String type,String account) {
        ...
        Page<BusinessAccountModel> page = mapper.findAllPage(type,account);
        result.setSuccess(true);
        if (page.size() > 0)
            result.setCode(200);
        else
